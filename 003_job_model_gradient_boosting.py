...
logging_module_name = "003_job_model_gradient_boosting"
logging_level = logging.DEBUG

logger = log.getLogger(logging_module_name, logging_level)

log.print_phase(logger, "003. MODELE GRADIENT BOOSTING sur jeu de données JOB")
...
preprocessing_GB = ColumnTransformer(
    transformers=[
        # ('drop', 'drop', ['WD_N2_']), # BKU : temporaire
        ('numeric', "passthrough", make_column_selector(dtype_include='number')),
        # ('category', OneHotEncoder(sparse_output=False, handle_unknown='ignore'), make_column_selector(dtype_include=['category','bool'])),
...
# --------------------
# . Données : pipeline feature + preprocessing
...
# Scores
# ------
logger.debug(f'score sur job_X_Train: {accuracy_score(y_train, optimised_GB.predict(X_train))}')
...
# ---------------------------
logger.info("003_job_fitted_gradient_boosting.joblib")
dump(optimised_GB, "./models/003_job_fitted_gradient_boosting.joblib")
logger.info("")  
# %%
