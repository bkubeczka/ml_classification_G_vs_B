...
logging_module_name = "004_main_best_model"
logging_level = logging.INFO

logger = log.getLogger(logging_module_name, logging_level)

log.print_phase(logger, "004. CHOIX du MEILLEUR MODELE pour le jeu de données MAIN (main_best_model)")
...
# =================
# LISTE des MODELES pour le jeu de données MAIN
# =================

log.print_section(logger, "Chargement des modèles pour le jeu de données MAIN")
...
for nom_fichier in os.listdir(repertoire_models):
    ...
    if nom_fichier.startswith('003_main_fitted') and nom_fichier.endswith('.joblib'):
...
X_train = load(f"{cst.MODELS_DIR}/{cst.JOBLIB_002_MAIN_X_TRAIN}")
y_train = load(f"{cst.MODELS_DIR}/{cst.JOBLIB_002_MAIN_Y_TRAIN}")
resample = load(f"{cst.MODELS_DIR}/{cst.JOBLIB_002_MAIN_RESAMPLE}")
...
logger.info(f'MAIN Best model : {best_model_name} (score={scores[best_model_name]})')
logger.info('\n')

# Sérialisation du nom de fichier pour étape suivante (prédiction)
# -------------------------------
dump(best_model_name, f"{cst.MODELS_DIR}/{cst.JOBLIB_002_MAIN_BEST_MODEL}")
